...
void saveload(PhoneBook* pb, int mode) {
    ...
    if (mode == 0) {
        ...
        char line[BUFFER_SIZE * 3]; // 한 줄을 저장할 버퍼
        while (fgets(line, sizeof(line), fp) != NULL) {
            ...
            strcpy(pb->entries[pb->n].studentNumber, token);
            ...
            pb->n++; // PhoneBook에 저장되어있는 인원 수 증가
        }
        fclose(fp); // 파일 닫기
    }
    // 데이터 저장 모드
